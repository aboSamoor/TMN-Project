...
The more the two queries appear together the smaller the distance between them. GND is not an accurate function to measure the distance. Moreover  GND is not a metric nor it satisfies the triangle inequality. First, $GND(x, y)$ is not larger than 0 for every $x \neq y$ \cite{DBLP:NGD}. 
